...
Provides TTS abstraction supporting ElevenLabs Multilingual & Flash v2.5, Cartesia Sonic,
...
# Active Free & Studio Voice IDs for ElevenLabs
DEFAULT_VOICES = {
    LanguageCode.ENGLISH: {
        "elevenlabs": "EXAVITQu4vr4xnSDxMaL",  # Sarah - Warm, professional educator
        "cartesia": "a0e99841-438c-4a64-b679-ae501e7d6091",
    },
    LanguageCode.HINGLISH: {
        "elevenlabs": "EXAVITQu4vr4xnSDxMaL",  # Sarah - Natural Hinglish / English blend
...
class VoiceEngine:
    ...
    def parse_script_to_ssml(
        ...
        3. Dominant emotion label
        4. Speech rate multiplier
        """
        # 1. Detect emotion tag
        emotion_match = re.search(r"<emotion=([a-zA-Z0-9_-]+)>", script)
        emotion = emotion_match.group(1).lower() if emotion_match else "enthusiastic"
        ...
        # 3. Convert to SSML
        ssml_body = script
        ssml_body = re.sub(r"<pause=([0-9]+)ms>", r'<break time="\1ms"/>', ssml_body)
        ssml_body = re.sub(r"<emotion=enthusiastic>(.*?)</emotion>", r'<prosody rate="105%" pitch="+2st">\1</prosody>', ssml_body, flags=re.DOTALL)
        ssml_body = re.sub(r"<emotion=thoughtful>(.*?)</emotion>", r'<prosody rate="95%" pitch="-1st">\1</prosody>', ssml_body, flags=re.DOTALL)
        ssml_body = re.sub(r"<emotion=encouraging>(.*?)</emotion>", r'<prosody rate="100%" pitch="+1st">\1</prosody>', ssml_body, flags=re.DOTALL)
        ssml_body = re.sub(r"<emotion=curious>(.*?)</emotion>", r'<prosody rate="102%" pitch="+3st">\1</prosody>', ssml_body, flags=re.DOTALL)
        ssml_body = re.sub(r"<emotion=empathetic>(.*?)</emotion>", r'<prosody rate="88%" pitch="-1st">\1</prosody>', ssml_body, flags=re.DOTALL)
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(url, json=payload, headers=headers, timeout=10) as resp:
                        if resp.status == 200:
                            ...
                            logger.info(f"ElevenLabs synthesized {len(audio_data)} audio bytes successfully!")
                            return base64.b64encode(audio_data).decode("utf-8")
                        else:
                            error_body = await resp.text()
                            logger.warning(f"ElevenLabs TTS status {resp.status}: {error_body}")
            except Exception as e:
                logger.warning(f"ElevenLabs TTS call failed: {e}")

        # 2. Fallback: Return None to let client-side Web Speech API play
        return None
